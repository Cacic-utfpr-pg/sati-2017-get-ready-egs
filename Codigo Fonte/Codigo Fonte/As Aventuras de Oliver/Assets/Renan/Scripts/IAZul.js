...
function Update () 
{
	...
	{		
		...
		if(auxMudarDir < 3 && distOliver <= 20 && distOliver >= 1.5 && pausa == 0 && distAlcance < distanciaMax-1 && voltando == false)
		{
			pausa = 0;
			if(tempoMudarDir >= 4)	
			{
		...
		{
			if((distOliver <= 20 && distOliver >= 1.5 && tempoAnimEsperando < 5 || (pausa > 11 && pausa <= 12)) && voltando == false)
			{
				if(pausa == 0)
				{
					pausa = 12;
			...
			{
				if(distOliver <= 20 && distOliver >= 1.5 && tempoAnimEsperando >= 5 || (pausa > 10 && pausa <= 11))
				{
					auxMudarDir = 0;
					pausa = 0;
					tempoAnimEsperando = 0;
				}
				//vai cair, então volta
				if(distAlcance > distanciaMax-1 || voltando == true) 
				{
					voltando = true;
					if(distAlcance < distanciaMax/2 ) 
					{
						voltando = false;
						auxMudarDir = 0;
						pausa = 0;
					...
					if(alcance == null) transform.LookAt(Vector3(oliver.transform.position.x, transform.position.y, oliver.transform.position.z));
					else transform.LookAt(Vector3(alcance.transform.position.x, transform.position.y, alcance.transform.position.z));
					print("entrei");
		...
		if((distOliver > 20 || (pausa > 12 && pausa <= 13)) || (distAlcance > distanciaMax-1))
		{
			tempoAnimEsperando = 0;
			pausa = 13;
		...
		{
			print("passei");
			if(voltando == true) transform.Translate(0,0,velMov);
